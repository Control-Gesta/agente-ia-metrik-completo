...
import ShadowLab from '@/components/ShadowLab'
        ...
        <>
          {modo === 'inicio' && <section>
            <SectionTitle
              ...
              description="Abra uma ferramenta por vez. Você sempre poderá voltar para esta escolha."
            />
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              <button
                type="button"
                onClick={() => { setModo('ensinar'); setEvals(null) }}
                className="panel interactive-card text-left p-4 min-h-[132px]"
              >
              ...
              <button
                type="button"
                onClick={() => { setModo('mapa'); setEvals(null) }}
                className="panel interactive-card text-left p-4 min-h-[132px]"
              >
                <Network size={17} className="text-purple-400 mb-4" />
                <div className="text-[14px] font-medium text-ink">Mapa Vivo</div>
                <p className="text-[12.5px] text-body-muted leading-relaxed mt-1">Veja regras, conteúdo, exemplos, mídias e tools conectados.</p>
              </button>

            </div>
            {admin && (
              <details className="panel group overflow-hidden mt-4">
                <summary className="list-none cursor-pointer px-5 py-4 flex items-center gap-3 hover-raise">
                  ...
                  <span className="text-[13px] font-medium text-ink flex-1">Ferramentas da agência</span>
                  <span className="text-[11px] text-body-faint">2 ferramentas protegidas</span>
                  <span className="text-[11px] text-body-faint group-open:rotate-90 transition-transform">▷</span>
                </summary>
                <div className="grid md:grid-cols-2 gap-3 p-4 border-t border-line-soft">
                  <button
                    type="button"
                    onClick={() => { setModo('laboratorio'); setEvals(null) }}
                    className="surface-alt interactive-card text-left p-4 rounded-[8px] border border-line-soft min-h-[118px]"
                  >
                    <TestTubes size={17} className="text-purple-400 mb-4" />
                    <div className="text-[14px] font-medium text-ink">Shadow Lab</div>
                    <p className="text-[12px] text-body-muted leading-relaxed mt-1">Compare uma regra candidata com conversas reais.</p>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <p className="text-[12px] text-body-muted leading-relaxed mt-1">Editar, examinar e publicar uma versão completa.</p>
                  </button>
                </div>
              </details>
            )}
          ...
          {modo !== 'inicio' && (
            <div className="flex items-center justify-between gap-4 border-b border-line-soft pb-4">
              <button
                type="button"
                onClick={() => { setModo('inicio'); setEvals(null); setPar(null) }}
                className="inline-flex items-center gap-2 text-[12.5px] text-body-muted hover:text-ink transition-colors"
              >
                <ArrowLeft size={14} /> Todas as opções
              </button>
              <span className="font-mono text-[9px] uppercase tracking-[0.13em] text-body-faint">
          ...
          {modo === 'ensinar' && (
            <section className="space-y-6">
              <div aria-label="Como ensinar a IA" className="grid sm:grid-cols-2 xl:grid-cols-4 gap-px overflow-hidden rounded-[10px] border border-line-soft bg-[var(--line-soft)]">
                {JORNADA.map(item => (
                  <div key={item.numero} className="bg-[var(--surface)] px-4 py-3.5">
                    <div className="flex items-center gap-2.5">
                      <div className="w-6 h-6 rounded-full border border-cyan/30 bg-cyan/[0.08] text-cyan text-[11px] font-mono flex items-center justify-center shrink-0">
                        {item.numero}
                      </div>
                      <div className="text-[13px] font-medium text-ink">{item.titulo}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {modo === 'mapa' && <KnowledgeMap promptText={original} />}

          {modo === 'laboratorio' && admin && <ShadowLab promptText={original} />}
